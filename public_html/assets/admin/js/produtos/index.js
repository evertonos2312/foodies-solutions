$(function () {
    $("#pesquisa_query").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: app_url + 'admin/produtos/procurar',
                data: {
                    'term': request.term,
                },
                dataType: 'json',
        ...
        select: function (event, ui) {
            if (ui.item.value === -1) {
                ...
                window.location.href = app_url + 'admin/produtos/show/' + ui.item.id;
            }
        }
    })
});

function excluirProduto(produto_id, produto_nome)
{
    var csrfName = $('.txt_csrfname').attr('name');
    var csrfHash = $('.txt_csrfname').val();
    let id_produto = produto_id;
    Swal.fire({
        title: 'Deletar produto - ' + produto_nome + '?',
