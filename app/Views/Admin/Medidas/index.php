...
<div class="content-header">
    ...
    <div>
        <a href="{$app_url}admin/medidas/criar" class="btn btn-primary"><i class="material-icons md-plus"></i> Criar Novo</a>
    </div>
</div>
{$breadcrumbs}
        ...
        <div class="table-responsive">
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{$medida['id']}</td>
                        <td><b><a class="" href="{$app_url}admin/medidas/show/{$medida['id']}">{mb_ucfirst($medida['nome'])}</a></b></td>
                        <td>{toDataBR($medida['criado_em'], true)}</td>
                        <td><span class="badge rounded-pill {$medida['ativo_class']}">{$medida['ativo']}</span></td>
                        <td class="text-center">
                            <a class="btn btn-light" id="editar_anchor_{$medida['id']}" href="{$app_url}admin/medidas/editar/{$medida['id']}">Editar</a>
                            <button class="btn btn-light text-danger" onclick="excluirMedida('{$medida.id}', '{$medida.nome}')">Excluir</button>
                        </td>
                    </tr>
                    {/foreach}
                    ...
                    {else}
                        <tr>
                            <td colspan="6">Nenhum medida encontrado</td>
                        </tr>
                    {/if}
...
<script src="{$app_url}assets/admin/js/medidas/index.js" type="text/javascript"></script>